package playzone.tj.presentation.main.home
...
import playzone.tj.presentation.main.home.adapter_home.EventAdapter
import playzone.tj.presentation.main.home.adapter_home.GenreAdapter
import playzone.tj.presentation.main.home.viewModels.HomeViewModel
import playzone.tj.presentation.main.home.viewModels.UserGenreUIState
import playzone.tj.presentation.main.home.viewModels.UserUIState
...
class HomeFragment : Fragment() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                homeViewModel.eventUIState.collectLatest { state ->
                    binding.apply {
                        mainProgressbar.isVisible = state is DomainEventState.Loading
                        rcViewEvents.isVisible = state is DomainEventState.Success
                        eventsImageError.isVisible =
                            (state is DomainEventState.Error || state is DomainEventState.Empty)
                        eventsTextError.isVisible =
                            (state is DomainEventState.Error || state is DomainEventState.Empty)
                        showAllText.isVisible = state is DomainEventState.Success
                        popularEventText.isVisible = state is DomainEventState.Success
                    }

                    when (state) {
                        is DomainEventState.Loading -> Unit
                        is DomainEventState.Success -> initEventRcView(state.eventData)
                        is DomainEventState.Error -> binding.eventsTextError.text = state.errorMessage
                        is DomainEventState.Empty -> binding.eventsTextError.text = "No such data"
                    }
                }
            }
        }
    ...
    private fun initEventRcView(list: List<DomainEventDTO>) {
        list.let {
